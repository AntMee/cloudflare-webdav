...
管理员后台使用登录接口获取会话 token。token 只在登录响应中返回一次，D1 中只保存 token 的哈希。
...
密钥不写入源码。初始管理员凭据通过 Cloudflare Secret 或一次性 bootstrap 流程设置。

密码校验使用 Workers 可用的 Web Crypto API。token 比较使用防时序攻击的比较逻辑。日志不能包含密码、会话 token 或 Basic Auth 请求头。
